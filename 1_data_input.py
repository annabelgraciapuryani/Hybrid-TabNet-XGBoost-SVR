...
if uploaded_file is not None:
    ...
    default_tanggal = next(
        (c for c in all_columns if "tanggal" in c.lower() or "date" in c.lower()),
        all_columns[0],
    )
    ...
    default_predictors = [c for c in numeric_cols if c not in [col_tanggal, col_target]]
